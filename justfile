...
data start="2026-02-02" end="2026-04-30" max_workers="4" max_jp_dates="35": _require-external-uv-env
    {{cli}} data-expansion --market all --start "{{ start }}" --end "{{ end }}" --tickers "{{ us_tickers }}" --max-workers "{{ max_workers }}" --max-jp-option-dates "{{ max_jp_dates }}"

benchmark-a1 *args: _require-external-uv-env
    @set -- {{args}}; \
    mask="stratified"; out="reports/runs/benchmark_a1"; seeds="1,2,3"; epochs="20"; batch_size="8"; max_nodes="250"; torch_threads="6"; device="auto"; baseline_preset="fast"; no_arb_surfaces="100"; variant_suite="core"; variants=""; skip_ood="false"; early_stopping_patience="0"; early_stopping_min_delta="0.0"; \
    for arg in "$@"; do \
      case "$arg" in \
        mask=*) mask="${arg#mask=}" ;; \
        out=*) out="${arg#out=}" ;; \
        ...
        skip_ood=*) skip_ood="${arg#skip_ood=}" ;; \
        skip_ood_predictions=*) skip_ood="${arg#skip_ood_predictions=}" ;; \
        early_stopping=*) early_stopping_patience="${arg#early_stopping=}" ;; \
        early_stopping_patience=*) early_stopping_patience="${arg#early_stopping_patience=}" ;; \
        early_stopping_min_delta=*) early_stopping_min_delta="${arg#early_stopping_min_delta=}" ;; \
